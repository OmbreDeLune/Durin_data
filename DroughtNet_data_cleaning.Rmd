...
mean_thickness <- rowMeans(thickness_data, na.rm = TRUE) # 'na.rm = TRUE' will ignore NAs in rows and calculate mean from available values
...
#ensure that DroughNet_plotID is numeric and siteID is a factor or character.
...
droughtnet_data <- droughtnet_data %>%
  mutate(
    ...
    DroughtTrt = case_when(
      ...
      # Default case to keep existing DroughtTrt values
      TRUE ~ DroughtTrt
    )
  )

view(droughtnet_data)
# all the meta data now matches the correct 7.1 and 7.3 plots

#inspect the data for overlapping plot ids

# Filter for rows where siteID is "Tjøtta" and ageClass is "Mature"
filtered_data <- droughtnet_data %>%
  filter(siteID == "Tjøtta", ageClass == "Mature")

view(filtered_data)


filtered_data <- droughtnet_data %>%
  filter(siteID == "Tjøtta", ageClass == "Pioneer")

view(filtered_data)  
# the data has overlap in the DroughNet_plotID for Tjøtta site

# Now to rectify this
# Reassign DroughNet_plotID values based on specified rules for Tjøtta and Mature
#so the reassigning will start from 10.1 as a continuation from lygra to 15.3 for the ambient and extreme drought plots that were sampled
droughtnet_data <- droughtnet_data %>%
  mutate(DroughNet_plotID = case_when(
    siteID == "Tjøtta" & ageClass == "Mature" & DroughNet_plotID == "1.2" ~ "10.1",
    siteID == "Tjøtta" & ageClass == "Mature" & DroughNet_plotID == "1.3" ~ "10.3",
    siteID == "Tjøtta" & ageClass == "Mature" & DroughNet_plotID == "2.1" ~ "11.1",
    siteID == "Tjøtta" & ageClass == "Mature" & DroughNet_plotID == "2.2" ~ "11.2",
    siteID == "Tjøtta" & ageClass == "Mature" & DroughNet_plotID == "3.1" ~ "12.1",
    siteID == "Tjøtta" & ageClass == "Mature" & DroughNet_plotID == "3.2" ~ "12.2",
    TRUE ~ as.character(DroughNet_plotID) # Keep original ID for other cases
  ))
...
droughtnet_data <- droughtnet_data %>%
  mutate(DroughNet_plotID = case_when(
    siteID == "Tjøtta" & ageClass == "Pioneer" & DroughNet_plotID == "1.2" ~ "13.2",
    siteID == "Tjøtta" & ageClass == "Pioneer" & DroughNet_plotID == "1.3" ~ "13.3",
    siteID == "Tjøtta" & ageClass == "Pioneer" & DroughNet_plotID == "2.2" ~ "14.2",
    siteID == "Tjøtta" & ageClass == "Pioneer" & DroughNet_plotID == "2.3" ~ "14.3",
    siteID == "Tjøtta" & ageClass == "Pioneer" & DroughNet_plotID == "3.2" ~ "15.2",
    siteID == "Tjøtta" & ageClass == "Pioneer" & DroughNet_plotID == "3.3" ~ "15.3",
    TRUE ~ as.character(DroughNet_plotID) # Keep original ID for other cases
  ))

view(droughtnet_data)

#remove columns unrelated to droughtnet 
# Remove the specified columns from the DataFrame
columns_to_remove <- c("habitat", "plotNR", "DURIN_plot", "stomatal_conductance.gs.", "spectrometry", "cutout_barcode", "new_taxon", "flag", "remark_dry_weighing", "notes", "flag_DryMass", "remarks_2")
droughtnet_data <- select(droughtnet_data, -all_of(columns_to_remove))
