...
#Step 2: Extract the envelope_ID values for plants in the "0.0" plot
envelopes_in_zero_plot <- calluna_data %>%
  filter(DroughNet_plotID == "0.0") %>%
  select(envelope_ID) %>%
  distinct()

# Display the envelope_IDs associated with DroughNet_plotID '0.0'
view(envelopes_in_zero_plot)
```
...
```{r}
# Filter entries where DroughNet_plotID is '0.0'
envelopes_in_zero_plot <- calluna_data %>%
  filter(DroughNet_plotID == "0.0") %>%
  select(envelope_ID) %>%
  distinct()
