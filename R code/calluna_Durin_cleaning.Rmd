...
new_heights <- data.frame(
  ...
  new_height = c(61.8, 34, 19.8, 32.6, 32.6, 32.6, 29.8, 29.8, 28.9, 28.9, 28.9, 30.5, 23.9, 30, 16.9)
)

# Join the new heights with the original calluna_data
calluna_data <- calluna_data %>%
  left_join(new_heights, by = "envelope_ID") %>%
  mutate(
    plant_height = case_when(
      !is.na(new_height) ~ new_height,  # When new_height is  plant_height
      TRUE ~ plant_height               # Otherwise, keep the original plant_height
    )
  ) %>%
  select(-new_height)  # Remove the new_height column after updating


#check if the issue of multiple heights  persists in the data

# Group by envelope_ID and summarize the number of unique heights
height_summary <- calluna_data %>%
  group_by(envelope_ID) %>%
  summarize(num_unique_heights = n_distinct(plant_height))

# Filter for IDs with more than one unique height
multiple_heights <- height_summary %>%
  filter(num_unique_heights > 1)

# Check if there are any IDs with multiple heights
if(nrow(multiple_heights) > 0) {
  cat("Some envelope_IDs have multiple heights:\n")
  print(multiple_heights)

  # Optional: Inspect specific cases
  # calluna_data %>%
  #   filter(envelope_ID %in% multiple_heights$envelope_ID) %>%
  #   arrange(envelope_ID)
} else {
  cat("No multiple heights issue found.")
}
#no multiple heights issue found 
...
```{r}
#update plant numbers
# Create a data frame of the updates to make
plant_nr_updates <- data.frame(
  envelope_ID = c("FLJ9637", "FGH3419", "JUK8367", "JUC3901"),
  new_plant_number = c(3, 1, 1, 2)
)

# Use mutate with case_when to update the plant_number
calluna_data <- calluna_data %>%
  left_join(plant_nr_updates, by = "envelope_ID") %>%
  mutate(
    plant_nr = case_when(
      !is.na(new_plant_number) ~ new_plant_number,
      TRUE ~ plant_nr                             
    )
  ) %>%
  select(-new_plant_number)  # Remove the new_plant_number column after updating
...
# correct calluna shoot type
calluna_data <- calluna_data %>%
  mutate(calluna_shoot_type = ifelse(envelope_ID == "JVA1966" & (is.na(calluna_shoot_type) | calluna_shoot_type == ""),  "Long", calluna_shoot_type))
...
plot_update <- data.frame(
  envelope_ID = c("JSB2142", "JRJ3443", "EZW2868", "FAY0434", "FCR0014", "FCU0944", "FCP8702", "JIQ4468", "JEU2375"), # envelope ids with wrong plot numbers
  new_DURIN_plot = c("Ly_F_CV_4", "Ly_F_CV_3", "Se_F_CV_2", "Se_F_CV_2", "Se_O_CV_4", "Se_O_CV_1", "Se_F_CV_4", "So_O_CV_2", "So_O_CV_5"),  # New DURIN_plot IDS
  new_plot_nr = c(4, 3, 2, 2, 4, 1, 4, 2, 5)  # New plot_nr numbers
)
