...
names(raw_data)[names(raw_data) == 'X..branches.5mm'] <- 'nbr_branches_lger_5mm'
names(raw_data)[names(raw_data) == 'X..broken_branches.5mm'] <- 'nbr_brk_branches_lger_5mm'
names(raw_data)[names(raw_data) == 'X..broken_branches.5mm.1'] <- 'nbr_brk_branches_shtr_5mm'
...
As we didn't have any fruits in S2 Y1 B2, B3 and B4, we can rename the fruit biomass from S2 Y1 B1 as "fruit_mass_s2_y1_side_branches"
```{r}
names(raw_data)[names(raw_data) == 'fruit_s2_y1_b1'] <- 'fruit_mass_s2_y1_side_branches'
```
...
```{r}
raw_data$detached_leaf_y1[is.na(raw_data$detached_leaf_y1)]<-0
raw_data$leaf_mass_s1_y1_b1<-raw_data$leaf_mass_s1_y1_b1+raw_data$detached_leaf_y1
...
raw_data$fruit_mass_s1_y1_b1<-raw_data$fruit_mass_s1_y1_b1+raw_data$s1_reproductive_part
...
raw_data$fruit_mass_s2_y1_side_branches <- raw_data$fruit_mass_s2_y1_side_branches %>% replace(is.na(.), 0)
raw_data$total_main_repro_biomass<-raw_data$reproductive_mass_s1_y1_b1+raw_data$reproductive_mass_s2_y2_b1+raw_data$fruit_mass_s2_y1_side_branches
...
```{r}
write.csv(half_cleaned_data, "C:\\Users\\ombre\\Desktop\\half_cleaned_DURIN_data.csv", row.names=FALSE)
```
