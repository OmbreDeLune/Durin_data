...
growth_halfwide$alive_leaf_mass<-growth_halfwide$alive_leaf_mass%>% replace(is.na(.), 0)
growth_halfwide$stem_mass<-growth_halfwide$stem_mass%>% replace(is.na(.), 0)
```
...
I want to study the leaf/stem ratio. I have to calculate it and add a column for it.
```{r}
growth_halfwide$leaf_stem_ratio<-growth_halfwide$alive_leaf_mass/growth_halfwide$stem_mass
```

visualise this ratio: 
```{r}
plot<-ggplot(growth_halfwide[!(growth_halfwide$species=="VM" & growth_halfwide$stemID=="S2_Y2_B1"),], aes(x=stem_mass, y=leaf_stem_ratio, fill=species))+geom_point(aes(colour=species))+facet_grid(siteID~habitat)+theme_few()+ggtitle("leaf/stem biomass ratio")
plot
```
